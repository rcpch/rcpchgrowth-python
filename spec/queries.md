...
The v4.6.4 release proved that PyPI does not support `.github/workflows/python-publish.yml` as a nested reusable Trusted Publisher identity: token exchange or artifact attestation verification fails because GitHub identifies `.github/workflows/release-on-merge.yml` as the top-level workflow. Publishing therefore runs in the top-level workflow, while the reusable workflow retains the test and single-build gates. Confirm whether a protected `pypi` environment should be introduced later; adding one requires updating PyPI's trusted-publisher configuration and any required environment reviewer would add a second human intervention after merge.
