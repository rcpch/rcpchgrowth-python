...
def measurement_from_sds(
    ...
    # elif reference == WHO:
    #     WHO references (2006+2007), when going from measurement -> SDS require a different method to calculate the measurement if the requested SDS is below -3 or above +3 SDS
    #     This involves calculating the measurement first to determine if it is below -3 or above +3 SDS and then interpolate between the distance between +2 and +3 SDS or -2 and -3 SDS points, as per the WHO method described here: https://cdn.who.int/media/docs/default-source/child-growth/growth-reference-5-19-years/computation.pdf?sfvrsn=c2ff6a95_4
    #    However: in the WHO own published centile curve values this rule is NOT applied and so therefore commented out here to reflect the actual centile curve values published by WHO, 
    #    which do not adjust for >3 or <-3 SDS values. This is because the WHO published centile curve values do not adjust for >3 or <-3 SDS values, and therefore neither should this function when generating centiles based on WHO references, to ensure consistency between the centiles generated by this function and the WHO published centiles.
...
def sds_for_measurement(
    ...
    if reference == CDC and measurement_method == BMI:
        ...
        if observation_value > m * (1 + l * s * 1.645)**(1/l):
            ...
            return z
    if reference == WHO and (
        ((measurement_method == BMI or measurement_method ==WEIGHT) and age >= 5)
        or age < 5
    ):
